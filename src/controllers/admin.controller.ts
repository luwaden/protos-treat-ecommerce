...
class AdminController {
  ...
  // =========================================================================
  getOnlineUsers = async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
  ...
  // =========================================================================
  toggleUserBan = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { id } = req.params;
      if (!id) throw new AppError("ID is required.", 400);
      const actingAdminId = (req as AuthRequest).user!.userId;

      if (id === actingAdminId) throw new AppError("You can't ban your own account.", 400);
  ...
  updateUserRole = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (id === actingAdminId) throw new AppError("You can't change your own role.", 400);
      if (!Object.values(UserRole).includes(role)) throw new AppError("Invalid role.", 400);
      ...
      if (target.role === UserRole.Admin && role === UserRole.Customer) {
        ...
        if (adminCount <= 1) throw new AppError("Can't remove the last remaining Admin.", 400);
      }
  ...
  getOrders = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const where = status && (status as string) !== "All" ? { orderStatus: status } : {};
  ...
  uploadProductImage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (!req.file) throw new AppError("No image file was provided.", 400);
  ...
  // =========================================================================
  deleteUploadedImage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
